...
def get_internet_routes():
    ...
    if data.status_code == 200:
        return list(mrtparse.Reader(gzip.GzipFile(fileobj=data.raw)))
    else:
        return []

def get_routes_from_mrt(ipv4=None, ipv6=None):
    if ipv4 and ipv6:
        print("Only ipv4 or ipv6 are allowed as they are mutually exclusive.")
    return []
...
for vrf in vni_map.keys():
    ...
    if networks:
        ...
        routes_ipv4 = get_routes_from_mrt()
        routes_ipv6 = get_routes_from_mrt()
            ...
            str(route): [
                {
                    ...
                    "metric":45,
                    "weight":0,
                    "path": " ".join(gen_as_path(sample=as_numbers)) if as_numbers else route,
                    "origin":"IGP",
                    "nexthops":[
                    {
                        "ip": "0.0.0.0",
            ...
            str(route): [
                {
                    ...
                    "metric":45,
                    "weight":0,
                    "path": " ".join(gen_as_path(sample=as_numbers)),
                    "origin":"IGP",
                    "nexthops":[
                    {
                        "ip": "0.0.0.0",
